...
class EmployeeScheduleUpdater
{
    ...
    private function updateEmployeeSchedule($employeeId)
    {
        ...
        if ($currentSchedule && $currentSchedule['schedule_id']) {
            ...
            // Soft delete assignments for these periods and sync to cloud
            foreach ($periodIds as $index => $pid) {
                $stmt = $this->db->prepare("UPDATE employee_assignments SET is_active = 0, sync_status = 0 WHERE schedule_period_id = ?");
            ...
            // Soft delete periods and sync to cloud
            $stmt = $this->db->prepare("UPDATE schedule_periods SET is_active = 0, sync_status = 0 WHERE schedule_id = ?");
            $stmt->bind_param('i', $oldScheduleId);
            $stmt->execute();
