...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('exportDTRBtn').addEventListener('click', function() {
    ...
    if (dateRange && dateRange.start_date && dateRange.end_date) {
      ...
      // Export with current month (default behavior)
...
async function loadDTRRecords(showAll = false) {
  ...
  try {
    ...
    if (dateRange && dateRange.start_date && dateRange.end_date) {
      // Use date range from calendar selection
      ...
      // Load all records without month filter
      fetchUrl = `api/dtr.php?action=get_employee_dtr&employee_id=${employeeId}`;
      if (!showAll) {
        fetchUrl += '&limit=10';
      }
    }
    ...
    if (result.success && result.data.records.length > 0) {
      ...
      // Show button if not showing all and there might be more records
      if (!showAll && result.data.records.length >= 10) {
...
async function loadDTRStats(employeeId, month = null) {
  try {
    let statsUrl = `api/dtr.php?action=get_stats&employee_id=${employeeId}`;
    if (month) {
      statsUrl += `&month=${month}`;
    }
