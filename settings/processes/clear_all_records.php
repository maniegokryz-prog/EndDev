...
class ClearRecordsProcessor
{
    ...
    private function clearAllRecords()
    {
        try {
            ...
            // First, attempt to clear the Kiosk Local DB via HTTP API
            $kioskResult = $this->clearKioskDatabase();
            if ($kioskResult) {
                $this->logActivity('Kiosk local database successfully cleared via local API');
            } else {
                $this->logError('Kiosk Local DB Clear', 'Failed to reach or clear Kiosk via localhost:5001. Local Kiosk logs might still exist.');
            }
    ...
    private function clearKioskDatabase()
    {
        // We use a short timeout because if the kiosk isn't running, we don't want the web app to hang
        $url = 'http://127.0.0.1:5001/api/clear_records';
        $options = [
            'http' => [
                'header' => "Content-type: application/json\r\n",
                'method' => 'POST',
        ...
        try {
            // Suppress warnings in case the server is offline
